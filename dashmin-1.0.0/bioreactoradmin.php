...
if (isset($_SESSION['user_id'])) {
    ...
    // Configuración de paginación para bioreactores
    $registros_por_pagina_bioreactor = 5; // Cambia este valor si deseas mostrar más registros por página
    $pagina_actual_bioreactor = isset($_GET['page_bioreactor']) ? (int) $_GET['page_bioreactor'] : 1;
    if ($pagina_actual_bioreactor < 1) $pagina_actual_bioreactor = 1;

    $offset_bioreactor = ($pagina_actual_bioreactor - 1) * $registros_por_pagina_bioreactor;

    // Consulta para obtener todos los bioreactores
    $sql_bioreactor = "SELECT c.id_bioreactor, 
                              c.nombre_bioreactor AS nombre_bioreactor, 
                              c.descripcion_bioreactor AS descripcion_bioreactor, 
                              c.direccion_ip, 
                              c.clave,
                              c.id_usuario  -- Incluye la columna id_usuario para mostrar el usuario asociado
                       FROM tabla_bioreactor c
                       ORDER BY c.nombre_bioreactor ASC 
                       LIMIT ? OFFSET ?"; 

    $bioreactores = $db->query($sql_bioreactor, [$registros_por_pagina_bioreactor, $offset_bioreactor]);

    // Consulta para obtener el total de bioreactores
    $total_bioreactor_query = "SELECT COUNT(*) as total FROM tabla_bioreactor";
    $total_bioreactor = $db->query($total_bioreactor_query);
    $total_paginas_bioreactor = ceil($total_bioreactor[0]['total'] / $registros_por_pagina_bioreactor);
} else {
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    <a href="./index.php" class="nav-item nav-link active"><i class="bi bi-clipboard-data"></i>Panel
                        Rapido</a><br />
                    <a href="./clientes.php" class="nav-item nav-link "><i
                            class="bi bi-clipboard-data"></i>Clientes</a><br />
                    <a href="./bioreactoradmin.php" class="nav-item nav-link "><i
                            class="bi bi-bar-chart-fill"></i>Bioreactores</a><br />
                    <a href="./graficos.php" class="nav-item nav-link "><i
                            class="bi bi-bar-chart-fill"></i>TDS</a><br />
                    <a href="./co2.php" class="nav-item nav-link "><i class="bi bi-bar-chart-fill"></i>cO2</a><br />
                    <a href="./tempagua.php" class="nav-item nav-link "><i class="bi bi-bar-chart-fill"></i>Temperatura
                        agua</a><br />
                    <a href="./tvoc.php" class="nav-item nav-link "><i class="bi bi-bar-chart-fill"></i>TVOC</a><br />
                    <a href="./bioreactor.php" class="nav-item nav-link "><i
                            class="bi bi-bar-chart-fill"></i>Bioreactor</a><br />
                        ...
                        <div class="card shadow mb-4">
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <h2>Tabla de Bioreactores</h2>
                                    <a href="./crear_bioreactor.php"><input type="button" class="btn btn-primary" value="Nuevo Bioreactor"  /></a>
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            <?php foreach ($bioreactores as $bioreactor): ?>
                                                <tr>
                                                    ...
                                                    <td><?= htmlspecialchars($bioreactor['id_usuario']) ?></td>
                                    ...
                                    <nav>
                                        <ul class="pagination">
                                            ...
                                            <?php endif; ?>
                                            <?php for ($i = 1; $i <= $total_paginas_bioreactor; $i++): ?>
                                                <li
                                                    class="page-item <?= ($i == $pagina_actual_bioreactor) ? 'active' : '' ?>">
                                                    <a class="page-link" href="?page_bioreactor=<?= $i ?>"><?= $i ?></a>
                                                </li>
                                            <?php endfor; ?>
                                            <?php if ($pagina_actual_bioreactor < $total_paginas_bioreactor): ?>
                                                <li class="page-item">
                                                    <a class="page-link"
